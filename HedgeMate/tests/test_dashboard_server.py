...
class DashboardServerTests(unittest.TestCase):
    ...
    def test_startup_refresh_launches_intraday_nowcast(self):
        ...
        self.assertEqual(payload["mode"], "intraday_nowcast")
    ...
    def test_load_dashboard_data_can_skip_execution_plan_price_enrichment(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            serve_dashboard.ROOT = root
            serve_dashboard.OUTPUT_PROCESSED_DIR = root / "outputs" / "processed"
            serve_dashboard.OUTPUT_REPORT_DIR = root / "outputs" / "reports"
            serve_dashboard.DOC_RESULT_DIR = root / "docs" / "STEP_1" / "04_?ㅽ뻾寃곌낵"
            run_id = "hedgemate-compact"
            write_csv(
                serve_dashboard.OUTPUT_PROCESSED_DIR / f"features_summary_{run_id}.csv",
                ["ticker", "mdd_1y_krw"],
                [{"ticker": "GLD", "mdd_1y_krw": "-0.1"}],
            ...
            write_csv(
                serve_dashboard.OUTPUT_REPORT_DIR / f"portfolio_multi_hedge_{run_id}.csv",
                ["candidate_combo", "recommendation_status", "hedge_share_counts"],
                [{"candidate_combo": "GLD + IEF", "recommendation_status": "PASS_RECOMMEND", "hedge_share_counts": "{\"GLD\": 1}"}],
            )
            write_csv(
                serve_dashboard.OUTPUT_REPORT_DIR / f"portfolio_1to1_hedge_{run_id}.csv",
                ["candidate_ticker", "recommendation_status", "hedge_share_counts"],
                [{"candidate_ticker": "GLD", "recommendation_status": "PASS_RECOMMEND", "hedge_share_counts": "{\"GLD\": 1}"}],
            )
            manifest = {"active_bundle": {"hedgemate_run": "different-run"}}

            with mock.patch.object(serve_dashboard, "parse_summary_markdown", return_value=({}, [])), mock.patch.object(
                serve_dashboard, "enrich_execution_plan"
            ) as enrich_mock:
                data = serve_dashboard.load_dashboard_data(run_id, product_manifest=manifest, include_execution_plan=False)

            enrich_mock.assert_not_called()
            self.assertEqual(data["portfolioOneToOne"][0]["candidate_ticker"], "GLD")
    ...
    def test_compact_product_dashboard_payload_trims_heavy_hedge_tables(self):
        dashboard = {
            "hedge": {
                "runId": "hedge-prod",
                "generatedAt": "2026-05-28T00:00:00Z",
                "meta": {"baseCurrency": "KRW"},
                "portfolioOneToOne": [{"candidate_ticker": f"H{i}"} for i in range(12)],
                "portfolioMulti": [{"candidate_ticker": f"M{i}"} for i in range(8)],
                "assetSensitivities": [{"ticker": f"A{i}"} for i in range(9)],
            },
            "hedgeActionPlan": [{"action_status": "REVIEW_ACTION"}],
            "hedgeActionCandidates": [{"candidate_ticker": "GLD"}],
        }

        compact = serve_dashboard.compact_product_dashboard_payload(dashboard, hedge_row_limit=3)

        self.assertTrue(compact["payloadCompact"]["enabled"])
        self.assertTrue(compact["hedge"]["compact"])
        self.assertEqual(compact["hedge"]["runId"], "hedge-prod")
        self.assertEqual(len(compact["hedge"]["portfolioOneToOne"]), 3)
        self.assertEqual(compact["hedge"]["rowCounts"]["portfolioOneToOne"], 12)
        self.assertEqual(compact["hedgeActionPlan"][0]["action_status"], "REVIEW_ACTION")
        self.assertEqual(compact["hedgeActionCandidates"][0]["candidate_ticker"], "GLD")
