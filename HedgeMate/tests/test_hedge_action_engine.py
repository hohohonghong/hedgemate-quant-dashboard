...
class HedgeActionEngineTests(unittest.TestCase):
    ...
    def test_action_generation_respects_formal_gate_and_no_auto_trim_offsets(self):
        ...
        self.assertAlmostEqual(float(add_action["linked_final_score"]), 0.8)
        self.assertEqual(int(add_action["user_display_score"]), 85)
        self.assertEqual(add_action["score_band"], "B:70-89")
        self.assertEqual(add_action["score_method_version"], "grade_banded_final_score_v1")
    ...
    def test_user_display_score_uses_grade_bands(self):
        cases = [
            ("A", 1.0, 100, "A:90-100"),
            ("B", 1.0, 89, "B:70-89"),
            ("C", 1.0, 69, "C:50-69"),
        ...
        for grade, final_score, expected_score, expected_band in cases:
            with self.subTest(grade=grade):
                payload = hedge_action_engine._score_contract_payload({"final_score": final_score}, grade)
                self.assertEqual(payload["user_display_score"], expected_score)
                self.assertEqual(payload["score_band"], expected_band)
                self.assertEqual(payload["score_method_version"], "grade_banded_final_score_v1")

    def test_action_quality_spreads_flat_same_grade_display_scores(self):
        rows = [
            {
                "action_id": "weak_b",
                "recommendation_grade": "B",
                "linked_final_score": 0.1,
        ...
        hedge_action_engine.apply_action_score_contracts(rows)
        ...
        self.assertLessEqual(by_id["strong_c"]["user_display_score"], 69)
        self.assertEqual(by_id["strong_c"]["score_band"], "C:50-69")

    def test_action_selection_keeps_grade_above_final_and_prescription_scores(self):
        def row(action_id, grade, final_score, prescription_score):
            item = {
                "action_id": action_id,
                "action_status": "REVIEW_ACTION",
                "recommendation_grade": grade,
                "vulnerability_delta": -0.10,
                "vulnerability_improve_pct": 10.0,
                "turnover_pct": 5.0,
                "prescription_score": prescription_score,
                "final_score": final_score,
            }
            item.update(hedge_action_engine._score_contract_payload(item, grade))
            return item

        weak_b = row("b_low_final", "B", 0.1, 10.0)
        perfect_c = row("c_high_final", "C", 1.0, 100.0)
        better_b = row("b_high_final", "B", 0.9, 20.0)

        ordered = sorted([perfect_c, weak_b, better_b], key=hedge_action_engine._action_selection_key)

        self.assertEqual([item["action_id"] for item in ordered], ["b_high_final", "b_low_final", "c_high_final"])
        self.assertLessEqual(perfect_c["user_display_score"], 69)

    def test_generic_benchmark_without_direct_match_stays_reference_grade(self):
        row = {
            "action_status": "REVIEW_ACTION",
        ...
        payload = hedge_action_engine._recommendation_grade_payload(row)

        self.assertEqual(payload["recommendation_grade"], "D")
        self.assertLessEqual(payload["user_display_score"], 49)
    ...
    def test_write_action_artifacts_outputs_required_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("linked_final_score", rows[0])
            self.assertIn("user_display_score", rows[0])
            self.assertIn("score_band", rows[0])
            self.assertIn("score_method_version", rows[0])
