...
GRADE_SCORE_BANDS = {
    RECOMMENDATION_GRADE_A: (90, 100),
    RECOMMENDATION_GRADE_B: (70, 89),
    RECOMMENDATION_GRADE_C: (50, 69),
    RECOMMENDATION_GRADE_D: (0, 49),
}
...
def _score_band_for_grade(grade):
    return GRADE_SCORE_BANDS.get(str(grade or "").strip().upper())


def _score_grade_for_contract(grade):
    normalized = str(grade or "").strip().upper()
    return normalized if normalized in GRADE_SCORE_BANDS else RECOMMENDATION_GRADE_D


def _score_band_label(grade):
    normalized = _score_grade_for_contract(grade)
    band = _score_band_for_grade(normalized)
    if not band:
        return ""
    return f"{normalized}:{band[0]}-{band[1]}"


def _linked_final_score(row):
    value = row.get("linked_final_score")
    if value in (None, ""):
        value = row.get("final_score")
    return _clip01(value)


def _user_display_score_for_grade(grade, final_score):
    band = _score_band_for_grade(grade)
    if not band:
        return None
    low, high = band
    return int(round(low + _clip01(final_score) * (high - low)))


def _score_contract_payload(row, grade):
    score_grade = _score_grade_for_contract(grade)
    linked_score = _linked_final_score(row)
    return {
        "linked_final_score": linked_score,
        "user_display_score": _user_display_score_for_grade(score_grade, linked_score),
        "score_band": _score_band_label(score_grade),
        "score_method_version": SCORE_METHOD_VERSION,
    }


def _set_score_contract(row):
    row.update(_score_contract_payload(row, row.get("recommendation_grade")))
    return row
...
def _recommendation_grade_payload(row):
    ...
    payload.update(_score_contract_payload(row, grade))
    return payload
...
def metric_fields_from_recommendation(row, action_type=None):
    metric = {
        ...
        "metric_coverage_reason": "",
        "linked_final_score": _clip01(row.get("linked_final_score") if row.get("linked_final_score") not in (None, "") else row.get("final_score")),
    }
...
def build_hedge_action_candidates(
    ...
    for sleeve in attribution_summary.get("risk_sleeves", []):
        ...
        sleeve_actions.sort(key=_action_selection_key)
...
def _dedupe_actions(rows):
    ...
    for row in rows:
        ...
        if current is None:
            deduped[key] = row
            continue
        challenger_score = _action_selection_key(row) + (-float(row.get("before_sleeve_vulnerability") or 0.0),)
        current_score = _action_selection_key(current) + (-float(current.get("before_sleeve_vulnerability") or 0.0),)
...
def _make_action(
    ...
    row = {
        ...
        "input_aware_score": rec_row.get("input_aware_score", ""),
...
def _action_score_value(row, *keys):
    for key in keys:
        value = _to_float(row.get(key), None)
        if value is not None:
            return value
...
def _action_selection_key(row):
    delta = _action_score_value(row, "vulnerability_delta")
    improve = _to_float(row.get("vulnerability_improve_pct"), None)
    if improve is None:
    ...
    return (
        ...
        delta,
        -float(improve or 0.0),
        -_action_score_value(row, "user_display_score", "userDisplayScore"),
        -_action_score_value(row, "linked_final_score", "linkedFinalScore", "final_score", "finalScore"),
        -_action_score_value(row, "prescription_score", "prescriptionScore"),
        _action_score_value(row, "turnover_pct", "estimated_turnover_pct", "turnoverPct"),
        -_action_score_value(row, "source_contribution_pct", "contribution_pct", "contributionPct"),
...
ACTION_QUALITY_COMPONENTS = [
    ("input_aware_score", ("input_aware_score",), 0.20, 1.0),
    ("vulnerability_improve_pct", ("vulnerability_improve_pct",), 0.35, 20.0),
    ("cvar_delta", ("expected_cvar_delta_after_cost", "cvar_delta"), 0.12, 0.003),
    ("mdd_delta", ("expected_mdd_delta_after_cost", "mdd_delta"), 0.12, 0.03),
    ("stress_delta", ("expected_stress_delta_after_cost", "stress_delta"), 0.11, 0.001),
    ("sharpe_delta", ("sharpe_delta",), 0.10, 0.05),
]


def _positive_component_value(row, keys):
    value = _metric_value(row, *keys)
    if value is None:
        return None
    return max(0.0, value)


def _component_score(value, values, fallback_scale):
    if value is None:
        return None
    ...
    return _clip01(value / fallback_scale if fallback_scale else value)


def _action_quality_scores(rows):
    values_by_name = {}
    for name, keys, _, _ in ACTION_QUALITY_COMPONENTS:
        values_by_name[name] = [_positive_component_value(row, keys) for row in rows or []]

    scores = {}
    for row in rows or []:
        components = []
        for name, keys, weight, fallback_scale in ACTION_QUALITY_COMPONENTS:
            value = _positive_component_value(row, keys)
            score = _component_score(value, values_by_name.get(name, []), fallback_scale)
        ...
        if weight_sum <= 0:
            continue
        scores[id(row)] = _clip01(sum(weight * score for weight, score in components) / weight_sum)
    return scores


def apply_action_score_contracts(action_rows):
    """Spread same-grade display scores when raw final_score is flat or missing."""
    quality_scores = _action_quality_scores(action_rows)
    for row in action_rows or []:
        raw_score = _linked_final_score({**row, "linked_final_score": row.get("raw_linked_final_score", row.get("linked_final_score"))})
        ...
        if quality_score is not None and raw_score <= 0.15:
            row["linked_final_score"] = round(quality_score, 6)
            row["score_driver_source"] = "action_quality_score"
        else:
            row["linked_final_score"] = raw_score
            row["score_driver_source"] = "linked_final_score"
        _set_score_contract(row)
    return action_rows
...
def build_hedge_action_plan(run_id, portfolio_weights, attribution_summary, action_rows):
    for row in action_rows:
        finalize_action_row_contract(row)
    apply_action_score_contracts(action_rows)
