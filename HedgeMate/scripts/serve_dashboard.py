...
def latest_running_market_refresh_job():
    with RUN_JOBS_LOCK:
        job_ids = [
            job_id
...
def launch_startup_market_refresh_if_needed(runner=subprocess.run, thread_factory=threading.Thread):
    ...
    if running_job:
        running_job["startupRefresh"] = True
...
def launch_refresh_market_data_job(payload=None, runner=subprocess.run, thread_factory=threading.Thread):
    ...
    running_job = latest_running_market_refresh_job()
    if running_job and mode != "portfolio_reanalysis" and not force:
        running_job["attachedToExisting"] = True
        return running_job
    freshness = {} if mode == "intraday_nowcast" else load_data_freshness()
    intraday_status = latest_intraday_nowcast_status() if mode == "intraday_nowcast" else None
    job_id = uuid.uuid4().hex
    with RUN_JOBS_LOCK:
        RUN_JOBS[job_id] = {
            "jobId": job_id,
    ...
    should_skip_latest = not force and (
        (mode == "market_data_only" and freshness.get("marketDataFresh"))
...
def build_data_freshness_note(display_date, data_date, snapshot_metadata):
    ...
    if lagging:
        ...
        return f"확정 일봉 레이어는 {preview}{suffix} 종가가 공통 확보된 {data_date} 기준입니다. 오늘 장중 흐름은 intraday nowcast에 반영됩니다."
    return f"확정 일봉 레이어는 공통 데이터 커버리지가 확보된 {data_date} 기준입니다. 오늘 장중 흐름은 intraday nowcast에 반영됩니다."
...
def build_recommendation_decision(hedge, backtest, data_freshness, event_status=None):
    ...
    if freshness_status == "stale":
        reason_text = "; ".join((data_freshness or {}).get("reasons") or []) or "시장/시나리오 기준일이 맞지 않습니다."
    ...
    if can_execute:
        ...
        copy = "그래도 헷지 후보는 현금 보유 기준, stress 표본, 데이터 기준일을 함께 확인한 뒤 실행해야 합니다."
        ...
        severity = "danger"
        title = "데이터 기준일이 맞지 않아 실행 추천을 차단했습니다."
        copy = "가격 데이터와 시나리오 스냅샷 기준일이 어긋난 상태에서는 좋은 후보처럼 보여도 실행 추천으로 취급하지 않습니다."
...
def enrich_optional_rows_contract(key, rows):
    ...
    if key in {"hedgeActionPlan", "hedgeActionCandidates"}:
        return [enrich_action_contract_aliases(row) for row in rows]
    return rows
...
def build_product_status(manifest, bundle, data_freshness, recommendation_decision, action_plan_decision, integrity):
    ...
    if (
        ...
        reasons.extend(data_freshness.get("reasons") or [])
    ...
    if freshness_status == "STALE" or str(data_freshness.get("status") or "").lower() == "stale" or data_freshness.get("needsRefresh"):
        reasons.extend(data_freshness.get("reasons") or manifest.get("stale_reasons") or bundle.get("stale_reasons") or [])
